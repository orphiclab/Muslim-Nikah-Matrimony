...
export default function OccupationPage() {
  ...
  useEffect(() => { setItems(loadMasterData().occupation); setMounted(true); }, []);

  const persist = (next: SimpleItem[]) => {
    setItems(next);
    const data = loadMasterData();
    saveMasterData({ ...data, occupation: next });
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <div className="flex items-center gap-2 text-sm text-gray-400 mb-1">
            ...
            <span>›</span><span className="text-gray-700 font-medium">Occupation</span>
          </div>
          <h1 className="text-[22px] sm:text-[26px] font-medium text-[#121514] flex items-center gap-2">💼 Occupation
        ...
        <div className="divide-y divide-gray-50">
          {adding && (
            <div className="px-5 py-3">
              <InlineInput placeholder="e.g. Software Engineer"
