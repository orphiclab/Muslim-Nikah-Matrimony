                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2.5">
                    <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2 w-full sm:w-auto">
                      ...
                      {isSubExpired(p) && (
                        <button
                          onClick={() => router.push(`/select-plan?profileId=${p.id}`)}
                          className="text-xs bg-red-600 text-white px-3.5 py-2 rounded-lg hover:bg-red-700 transition font-semibold flex items-center justify-center gap-1.5">
                      ...
                      {!isSubExpired(p) && (p.status === 'DRAFT' || p.status === 'EXPIRED') && (
                        p.rejectionReason && p.rejectionReason !== 'Payment not received' ? (
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-xs bg-green-600 text-white px-3.5 py-2 rounded-lg hover:bg-green-700 transition font-semibold flex items-center justify-center gap-1.5">
                          ...
                          <button
                            onClick={() => router.push(`/select-plan?profileId=${p.id}`)}
                            className="text-xs bg-[#1C3B35] text-white px-3.5 py-2 rounded-lg hover:bg-[#15302a] transition font-semibold flex items-center justify-center gap-1.5">
                            <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                      ...
                      {p.status === 'ACTIVE' && (
                        <a href="/dashboard/chat"
                          className="text-xs border border-[#1C3B35] text-[#1C3B35] px-3.5 py-2 rounded-lg hover:bg-[#1C3B35]/5 transition font-semibold flex items-center justify-center gap-1.5">
                          <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                      ...
                      <button
                        ...
                        className="text-xs border border-gray-200 text-gray-600 px-3.5 py-2 rounded-lg hover:bg-[#1C3B35]/5 hover:border-[#1C3B35] hover:text-[#1C3B35] transition font-semibold flex items-center justify-center gap-1.5">
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                          <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>
                        </svg>
                        View Details
                      ...
                      {!isSubExpired(p) && p.status !== 'PAYMENT_PENDING' && (
                        <button
                          ...
                          className="text-xs border border-gray-200 text-gray-600 px-3.5 py-2 rounded-lg hover:bg-purple-50 hover:border-purple-300 hover:text-purple-700 transition font-semibold flex items-center justify-center gap-1.5">
                          <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                    ...
                    <button onClick={() => setDeleteTarget(p)}
                      className="w-full sm:w-auto self-stretch sm:self-auto text-xs border border-gray-200 text-gray-500 px-3.5 py-2 rounded-lg hover:bg-red-50 hover:border-red-300 hover:text-red-600 transition font-semibold inline-flex items-center justify-center gap-1.5">
